...
contains 
   ...
   subroutine circ_harm(lmax, ang, blm)
      ...
      do l = 1, lmax
         blm(:, 2*l + 1) = blm(:, 2*l + 1) * cmplx(sin(l * ang(:)), 0.0, r64)
         blm(:, 2*l + 2) = blm(:, 2*l + 1) * cmplx(cos(l * ang(:)), 0.0, r64)
      end do
      !$omp end do
      !$omp end parallel 
   ...
   !> Notice that this function is intended to be used with a Gauss-Legendre grid of order 21 so the
   ...
   subroutine circ_harm_expansion(lmax, f, weights, blm, clm)
      ...
      complex(r64), intent(out)  :: clm(:,:)

      integer(i64)              :: i
      integer(i64), parameter   :: nr = 21_i64
      ...
      if (size(f,1) /= nr .or. size(ylm,1) /= nr .or. size(weights) /= nr) then
